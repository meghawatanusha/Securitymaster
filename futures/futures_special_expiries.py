...
def main(args):
    ...
    engine = create_engine('mysql+pymysql://bors_master:uJ!DicDu*7sW@bors-db1.inmu1.eglp.com:3306/futures_test')
    ...
    metadata.reflect(engine, only=['attributes','instruments','prices','prices_futures' ,'last_generated_egid','futureseries','open_interest_table','futures_contract_table'])
              ...
              for i in data:
                  if (i[0]== 'FUTIDX'):
                    ...
                    if ( not(underlying_fut in active_future_series) and (exist_query is not None and exist_query.count() < 1)):
                        ...
                        index_attribute = Attributes(**{'egid':future_id_counter, 'attribute':3,'attributevalue':underlying, 'startdate':datetime.datetime.strptime(i[14],'%d-%b-%Y').date() ,'enddate':default_enddate })
                        s1.add(index_attribute)

                        index_instrument = Instruments(**{'egid':future_id_counter,
                                             'instrument_type':4,
                                             'startdate':datetime.datetime.strptime(i[14],'%d-%b-%Y').date(),
                                             'enddate':datetime.datetime.strptime(i[14],'%d-%b-%Y').date()
                                          })
                        s1.add(index_instrument)   
