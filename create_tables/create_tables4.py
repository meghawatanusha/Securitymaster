...
class Prices(Base) :
    ...
    total_turnover = Column(DOUBLE,primary_key=True)
...
class Open_interest_table (Base) :
    ...
    trade_date=Column(DATETIME, primary_key=True,nullable=False)
    open_interest= Column(DOUBLE,primary_key=True)
    change_in_open_interest=Column(DOUBLE, primary_key=True)
...
#    db = create_engine('mysql+pymysql://egil_user:egil_user@egsecuritymaster.cvxyuzctu0dv.ap-south-1.rds.amazonaws.com:3306/eg8')
    db = create_engine('mysql+pymysql://bors_master:uJ!DicDu*7sW@bors-db1.inmu1.eglp.com:3306/futures_test')
